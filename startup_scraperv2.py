...
def trigger_scrape():
    ...
    logging.info("Manual trigger endpoint called")
    
    if scraper_instance is None or email_config_global is None:
...
def load_email_config():
    ...
    required_env_vars = ['SENDER_EMAIL', 'SENDER_PASSWORD', 'RECIPIENTS']
    missing_vars = [var for var in required_env_vars if not os.getenv(var)]
    
    if missing_vars:
        logging.error(f"Missing required environment variables: {', '.join(missing_vars)}")
        logging.error("Please set these environment variables before running the application.")
        return None
    ...
    if not email_config['recipients']:
        logging.error("No valid recipient emails found in RECIPIENTS environment variable")
        return None
...
def main():
    ...
    # Load email configuration from environment variables
    email_config_global = load_email_config()
    if email_config_global is None:
        return 1
    ...
    try:
        ...
        logging.info(f"Sender email: {email_config_global['sender_email']}")
        logging.info(f"Recipients: {email_config_global['recipients']}")
        logging.info(f"Monitoring {len(scraper_instance.sources)} sources")
    ...
    if args.mode == 'cloud':
        ...
        scraper_instance.daily_scrape_and_send(email_config_global)
...
if __name__ == "__main__":
    ...
    # Simple cloud detection for platforms like Heroku, Railway, etc.
    if os.environ.get("PORT") or '--mode' in sys.argv and 'cloud' in sys.argv:
        # Force cloud mode if PORT environment variable is set
        sys.argv.extend(['--mode', 'cloud'])
        if os.environ.get("PORT"):
            sys.argv.extend(['--port', os.environ.get("PORT")])
